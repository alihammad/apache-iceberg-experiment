...
# e.g., docker-compose logs -f spark
# e.g., docker-compose logs -f trino-coordinator
# e.g., docker-compose logs -f minio
ClassNotFoundException: org.apache.hadoop.fs.s3a.S3AFileSystem in Spark: Ensure SPARK_JARS_PACKAGES in docker-compose.yml correctly includes org.apache.hadoop:hadoop-aws and com.amazonaws:aws-java-sdk-bundle. The wget commands in your current docker-compose.yml for the Spark service are redundant if SPARK_JARS_PACKAGES is correctly configured and can be removed for a cleaner setup. Spark will download these packages automatically.
Trino Cannot Connect to MinIO or See Iceberg Tables:
Verify network connectivity between Trino and MinIO containers (they are on the same Docker network).
Double-check S3 credentials and endpoint in trino/catalog/iceberg.properties.
Ensure the iceberg.catalog.local.warehouse path in Spark matches where Trino is looking.
Check Trino coordinator logs for errors related to catalog loading or S3 access.
PySpark ModuleNotFoundError: No module named 'pyspark' or 'py4j': The apache/spark-py:v3.3.0 image comes with PySpark and py4j. Installing them again via pip (like pip install ... pyspark==3.3.0 py4j) can sometimes cause issues. It's generally better to rely on the versions provided by the base image. If you encounter this, try removing pyspark and py4j from the pip install line in the Spark service's command in docker-compose.yml.
Clean Start
To remove all containers, networks, and volumes (including MinIO data):

bash
docker-compose down -v
Component Versions
Apache Spark: 3.3.0 (from apache/spark-py:v3.3.0 image)
Apache Iceberg: 1.4.2 (specified in SPARK_JARS_PACKAGES and wget)
Trino: 419 (from trinodb/trino:419 image)
MinIO: Latest (from minio/minio image)
Hadoop AWS: 3.3.2 (for S3A filesystem, matches Spark's Hadoop version)
AWS Java SDK Bundle: 1.11.1026 (dependency for Hadoop AWS)
Port Mappings
8888:8888: Jupyter Notebook (Spark container)
8080:8080: Trino UI & API (Trino container)
9000:9000: MinIO S3 API (MinIO container)
9001:9001: MinIO Console (MinIO container). ├── docker-compose.yml ├── notebooks/ # Jupyter notebooks for Spark │ └── example.ipynb ├── trino/ │ ├── catalog/ # Trino catalog configuration │ │ └── iceberg.properties │ │ └── minio.properties │ └── etc/ # Other Trino configurations │ ├── config.properties │ └── jvm.config └── README.md
...
8080:8080: Trino UI & API (Trino container)
9000:9000: MinIO S3 API (MinIO container)
9001:9001: MinIO Console (MinIO container)
4040:4040: Spark UI (Spark container)
4040:4040: Spark UI (Spark container)